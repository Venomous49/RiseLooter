...
(() => {
  if (window.__RISELOOTER_CPX_V7__) return;
  window.__RISELOOTER_CPX_V7__ = true;
  ...
  function buildWallUrl(user,cfg){
    ...
    u.searchParams.set('subid_2','inline-wall');
  ...
  function ensureShell(missions){
    ...
    mount.style.cssText='margin-top:16px;padding-top:16px;border-top:1px solid #203141;min-height:420px';
    mount.innerHTML=`
      <div style="font-weight:900;font-size:20px;margin-bottom:6px">Sondages CPX Research</div>
      ...
      <div id="cpx-inline-title" style="display:none;margin:4px 0 10px;font-weight:800">Choisis directement une enquête ci-dessous :</div>
      <div id="cpx-frame-wrap" style="display:none;overflow:hidden;border:1px solid #25384a;border-radius:12px;background:#08131c">
        <iframe id="cpx-riselooter-frame" title="Sondages CPX Research" style="display:block;width:100%;height:2000px;border:0;background:#08131c" referrerpolicy="strict-origin-when-cross-origin" loading="eager" allow="camera; microphone"></iframe>
      </div>
      <div style="display:flex;gap:8px;flex-wrap:wrap;margin-top:10px"><a id="cpx-open-wall" class="btn dark" target="_blank" rel="noopener" style="display:none;font-size:13px">Ouvrir en plein écran</a></div>
  ...
  async function mountCPX(){
    ...
    try{
      ...
      const cfg=await getServerConfig(user),wallUrl=buildWallUrl(user,cfg);
      if(open){open.href=wallUrl;open.style.display='inline-block';}
      if(frame){if(frame.src!==wallUrl)frame.src=wallUrl;frame.dataset.cpxUser=String(user.id);}
      if(frameWrap)frameWrap.style.display='block';
      if(inlineTitle)inlineTitle.style.display='block';
