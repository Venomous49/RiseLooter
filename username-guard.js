(() => {
  ...
  async function syncAuthenticatedUsername(session){
    ...
    if(result.ok){try{if(typeof loadLeaderboard==='function')await loadLeaderboard();}catch(_){}}
  }

  function installAvailabilityHint(){
    const input=$('usernameInput');if(!input||$('usernameAvailability'))return;
  ...
  function protectLegacyClaim(){
    document.addEventListener('click',async event=>{
      ...
      event.preventDefault();event.stopImmediatePropagation();btn.disabled=true;const result=await claim(username);btn.disabled=false;
  ...
  async function boot(){
    patchExistingAccountSignup();installAvailabilityHint();protectSignup();protectLegacyClaim();
    const observer=new MutationObserver(()=>installAvailabilityHint());observer.observe(document.documentElement,{childList:true,subtree:true});
    try{const {data}=await sb.auth.getSession();await syncAuthenticatedUsername(data?.session);}catch(_){}
    try{sb.auth.onAuthStateChange((_event,session)=>setTimeout(()=>syncAuthenticatedUsername(session),150));}catch(_){}
  }
