(() => {
  ...
  function removeDuplicateAdminPanels(){
    const panels=[...document.querySelectorAll('#rlAdminPayouts')];
    ...
    adminMountPromise=(async()=>{
      try{
        ...
        wrap.querySelectorAll('[data-pay-id]').forEach(btn=>btn.addEventListener('click',async()=>{
          ...
          btn.disabled=true;
          try{
            const r=await api(`/api/admin/withdrawals/${btn.dataset.payId}/pay`,{method:'POST'});
            alert(`Paiement PayPal lancé. Statut : ${r.paypal_status||'envoyé'}`);
            await mountAdminPayouts(true);
  ...
  function boot(){
    ...
    const obs=new MutationObserver(()=>{
      interceptWithdrawal();
      removeDuplicateAdminPanels();
      if($('rlAdminOverlay')&&!$('rlAdminPayouts'))mountAdminPayouts();
    });
